<template>
          ...
          <div class="device-stats">
            ...
            <div class="device-item">
              <div class="health-card temp-health-card">
                <div class="health-title">CPU 温度</div>
                <div class="cpu-health-layout">
                  <div class="cpu-pie-box">
                    <div
                      class="temp-ring"
                  ...
                  <div class="cpu-temp-chart-area">
                    <canvas ref="cpuTempCanvas" class="cpu-temp-canvas"></canvas>
                  </div>
                </div>
              </div>
            </div>
  ...
  <el-dialog
    ...
    <div class="settings-panel">
      ...
      <!-- 性能模式、发射功率、国家码在 G5Pro 上暂不可用，已隐藏 -->
    </div>
    <template #footer>
...
const cpuTempCanvas = ref<HTMLCanvasElement | null>(null);
const cpuTempHistory = ref<number[]>([]); // 最近60个数据点
...
async function fetchAllData() {
  ...
  try {
    ...
    // 记录 CPU 温度历史
    const temp = Number(cpuTemp.value?.cpuss_temp) || 0;
    cpuTempHistory.value.push(temp);
    if (cpuTempHistory.value.length > 60) cpuTempHistory.value.shift();
    drawCpuTempChart();
...
function psmGetHandler() {
  axios.post('/api/wifi/psm/get', { ifaces: ['ra0', 'ra1', 'rai0', 'rai1'], })
    .then((res) => {
      if (res.data.code !== 0) return;
      const data = res.data.data;
      wifiInfo.value.ra0 = data.ra0_psm;
      wifiInfo.value.ra1 = data.ra1_psm;
...
// ─────────────────────────── CPU 温度折线图 ───────────────────────────
function drawCpuTempChart() {
  const canvas = cpuTempCanvas.value;
  if (!canvas) return;
  ...
  const W = rect.width;
  const H = rect.height;
  const data = cpuTempHistory.value;
  const maxPoints = 60;
  ...
  const toY = (t: number) => H - ((t - minT) / (maxT - minT)) * (H - 8) - 4;
  ...
  if (lastTemp >= 70) {
    grad.addColorStop(0, 'rgba(255,77,79,0.35)');
    grad.addColorStop(1, 'rgba(255,77,79,0.02)');
  } else if (lastTemp >= 55) {
    grad.addColorStop(0, 'rgba(255,165,0,0.3)');
    grad.addColorStop(1, 'rgba(255,165,0,0.02)');
  } else {
  ...
  if (lastTemp >= 70) lineColor = '#ff4d4f';
  else if (lastTemp >= 55) lineColor = '#ffa500';
  else lineColor = '#409eff';
...
// 协商速率：hostapd 的 rate 单位因频段而异
// 2.4G (ra0): rate / 10000 → Mbps（如 6000000 → 600 Mbps）
// 5G  (rai0/rai1): rate / 3125 → Mbps（如 6232000 → ~1922 Mbps）
function formatRate(rateMbps?: number): string {
  if (!rateMbps || rateMbps <= 0) return '-';
  return String(Math.round(rateMbps));
}
